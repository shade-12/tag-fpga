 ...
 *      Author: Shade Wong
 * 
 *  DMA test for seding data from HPS to FPGA 
 */
...
// HPS TO FPGA DMA is at offset 0x40 in Qsys
// but these offsets are in WORDS 
// 0x40 bytes = 64 bytes = 16 words
#define DMA_HPS_TO_FPGA_STATUS_OFFSET	16+0
#define DMA_HPS_TO_FPGA_READ_ADD_OFFSET	16+1 
#define DMA_HPS_TO_FPGA_WRT_ADD_OFFSET	16+2
#define DMA_HPS_TO_FPGA_LENGTH_OFFSET	16+3
#define DMA_HPS_TO_FPGA_CNTL_OFFSET		16+6
...
int main(void)
{
	...
	while(1) 
	{
		...
		*(DMA_base_ptr + DMA_HPS_TO_FPGA_STATUS_OFFSET) = 0;
		// read bus-master gets data from HPS addr=0xffff0000
		*(DMA_base_ptr + DMA_HPS_TO_FPGA_READ_ADD_OFFSET) = 0xffff0000 ;
		// write bus_master for fpga sram is mapped to 0x08000000 
		*(DMA_base_ptr + DMA_HPS_TO_FPGA_WRT_ADD_OFFSET) = 0x08000000 ;
		// copy 4000 bytes for 1000 ints
		*(DMA_base_ptr + DMA_HPS_TO_FPGA_LENGTH_OFFSET) = N*4 ;
		// set bit 2 for WORD transfer
		// set bit 3 to start DMA
		// set bit 7 to stop on byte-count
		// start the timer because DMA will start
		gettimeofday(&t1, NULL);
		*(DMA_base_ptr + DMA_HPS_TO_FPGA_CNTL_OFFSET) = 0b10001100;
		while ((*(DMA_base_ptr + DMA_HPS_TO_FPGA_STATUS_OFFSET) & LENGTH_ZERO_MASK) == 0) WAIT;
		// finish timing the transfer
		gettimeofday(&t2, NULL);
		elapsedTime = (t2.tv_sec - t1.tv_sec) * 1000000.0;      // sec to us
		elapsedTime += (t2.tv_usec - t1.tv_usec) ;   // us to 
